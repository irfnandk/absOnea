...
<div class="app">
    <aside class="sidebar">
        ...
        <a href="?action=admin&sub=jadwal" class="active">Jadwal</a>
    ...
    <main class="main">
        <div class="header">
            <h1>Data Jadwal</h1>
            <a href="?action=admin&sub=jadwal-create" class="btn-primary">+ Tambah Jadwal</a>
        </div>
        <table class="table-custom">
            <thead><tr><th>MK</th><th>Dosen</th><th>Hari</th><th>Jam</th><th>Ruang</th><th>Aksi</th></tr></thead>
            <tbody>
            <?php if (isset($data) && count($data) > 0): ?>
                <?php foreach ($data as $j): ?>
                <tr>
                    <td><?= htmlspecialchars($j['nama_mk']) ?></td>
                    <td><?= htmlspecialchars($j['dosen_nama']) ?></td>
                    ...
                    <td>
                        <a href="?action=admin&sub=jadwal-edit&id=<?= $j['id'] ?>" class="btn-edit">Edit</a>
                        <a href="?action=admin&sub=jadwal-delete&id=<?= $j['id'] ?>" class="btn-delete" onclick="return confirm('Hapus jadwal ini?')">Hapus</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px 0;">Belum ada data jadwal.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </main>
